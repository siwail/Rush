...
public class Stalker {
    ...
    public void math(){
        ...
        for (int i = 0;i<q;i+=1){
            ...
            if (state[i]==9) {
                vx[i] = (m.persik.x-x[i])/200f+m.random.nextInt(3)-1;
                vy[i] = (m.persik.y-y[i])/200f+m.random.nextInt(3)-1;
            }
            if (state[i]==10) {
                vx[i] = (m.persik.x-x[i])/80f+m.random.nextInt(5)-2;
                vy[i] = (m.persik.y-y[i])/80f+m.random.nextInt(5)-2;
                if (distance(x[i], y[i], m.persik.x, m.persik.y)<850){
            ...
            if (distance(x[i], y[i], m.persik.x, m.persik.y)<550){
                x[i] = m.random.nextInt((int)(m.fx*m.res*2));
                y[i] = m.random.nextInt((int)(m.fy*m.res*2));
                a[i] = m.random.nextInt(4);
                t[i] = m.random.nextInt(3);
    ...
    public void draw(){
        ...
        for (int i = 0;i<q;i+=1) {
            m.batch.draw(randomize(l[a[i]], distance(x[i], y[i], m.persik.x, m.persik.y)), -m.cx+x[i], -m.cy+y[i],scale/2f, scale/2f, scale, scale, -d[i], 1,  0);
            m.batch.draw(randomize(h[t[i]], distance(x[i], y[i], m.persik.x, m.persik.y)), -m.cx+x[i]+scale/4f*(d[i])+scale/4f, -m.cy+y[i]+scale*0.7f, scale/4f, scale/4f, scale/2f, scale/2f,d[i], 1,  rotation);

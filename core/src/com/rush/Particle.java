    ...
    public void math() {
        if (state == 1) {
            for (int i2 = 0; i2 < 3; i2 += 1) {
                sx += r.random.nextInt(3) - 1 - (x - r.persik.x) / 100f;
                sy += r.random.nextInt(3) - 1 - (y - r.persik.y) / 100f;
                sz += r.random.nextInt(3) - 1 - (z - r.persik.xz) / 100f;
                x += sx * speed;
                y += sy * speed;
                z += sz * speed;
                if (r.hit(x, y, r.persik.x + r.persik.w / 2, r.persik.y + r.persik.h / 2, 50, 50)) {
                    state = 0;
